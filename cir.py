...
# Step 2: Define the Cox-Ingersoll-Ross (CIR) Model
def cir_model(r, dt, a, b, sigma):
    return r + a * (b - r) * dt + sigma * np.sqrt(r) * np.sqrt(dt) * np.random.normal()

# Step 3: Define the Objective Function
def objective_function(params):
    a, b, sigma = params
    errors = []
    dt = 1  # assuming quarterly increments
    for i in range(len(data)-1):
        predicted_rate = cir_model(data['Rate'][i], dt, a, b, sigma)
        errors.append((data['Rate'][i+1] - predicted_rate) ** 2)
    return np.mean(errors)

# Step 4: Parameter Estimation
initial_guess = [0.1, 0.05, 0.01]  # initial parameter values
result = minimize(objective_function, initial_guess, method='Nelder-Mead')
estimated_params = result.x

# Step 5: Simulate Interest Rate Paths
...
for i in range(num_paths):
    for j in range(1, len(data)):
        simulated_rates[i, j] = cir_model(simulated_rates[i, j-1], dt, *estimated_params)

# Step 6: Derive Forward Rates
forward_rates = np.zeros((num_paths, len(data)-1))
for i in range(num_paths):
    for j in range(len(data)-1):
        forward_rates[i, j] = (1 / dt) * (np.exp(simulated_rates[i, j]) - 1)

mean_forward_rates = np.mean(forward_rates, axis=0)
maturities = np.arange(1, len(data))

# Step 7: Plot the Forward Curve with Dates on x-axis
plt.plot(data['Maturity'].iloc[:-1], mean_forward_rates)
plt.xlabel('Date')
plt.ylabel('Forward Rate')
plt.title('Cox-Ingersoll-Ross (CIR) Model Forward Curve')
...
trading_signals = trading_signals[:len(returns)]

# Iterate through the returns and calculate the portfolio value

for i in range(1, len(returns)-2):
