...
# Step 2: Define the Hull-White Model
def hull_white_model(r, dt, kappa, theta, sigma):
    a = kappa * theta
    b = sigma
    Z = np.random.normal()
    return r + a * dt + b * np.sqrt(dt) * Z
...
def objective_function(params):
    ...
    for i in range(len(data)-1):
        predicted_rate = hull_white_model(data['Rate'][i], dt, kappa, theta, sigma)
...
for i in range(num_paths):
    for j in range(1, len(data)):
        simulated_rates[i, j] = hull_white_model(simulated_rates[i, j-1], dt, *estimated_params)
...
plt.title('Hull-White Model Forward Curve')
