...
# Step 2: Define the Vasicek Model
def vasicek_model(r, dt, a, b, sigma):
    return r + a * (b - r) * dt + sigma * np.sqrt(dt) * np.random.normal()
...
def objective_function(params):
    ...
    for i in range(len(data)-1):
        predicted_rate = vasicek_model(data['Rate'][i], dt, a, b, sigma)
...
for i in range(num_paths):
    for j in range(1, len(data)):
        simulated_rates[i, j] = vasicek_model(simulated_rates[i, j-1], dt, *estimated_params)
...
plt.title('Vasicek Model Forward Curve')
